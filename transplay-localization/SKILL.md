...
1. **请求场景判定**：
   - **大批量或全局巡检场景**：主代理优先调用 `get_all_mods_status_tool` 获取整个库下所有 Mod 的状态看板。通过分析输出末尾 of `Check Summary` 提醒行，获知有哪些仅包含 `origin` 目录而遗留待翻的全新模组（Need Translation），或由于新覆盖发生变更 of 过时模组（Need Update），以此作为调度总看板来筛选和循环分派本批次的翻译任务。
   - **单 Mod 处理场景**：主代理无需调用全量列表扫描，直接通过检查目标 Mod 本地的物理目录及 Git 状态进行精准分流。
2. **分流决策规则 (单 Mod 判定级)**：
   - **分支 A (全新翻译)**：若该 Mod 在物理磁盘上仅存在 `origin/` 但 `translated/` 目录不存在。在主代理下执行 **[4. 全新翻译流程]**。
   - **分支 B (增量更新)**：若该 Mod 的 `translated/` 已存在，且主代理通过 Git 工具检测到其 `origin/` 目录存在未提交的工作区变更。在主代理下执行 **[5. 增量更新流程]**。
